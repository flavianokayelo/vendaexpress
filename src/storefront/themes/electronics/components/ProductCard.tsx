// ProductCard do tema Electronics — cards robustos com padding interno
// (padded-tint), bom espaço para nomes longos e leitura de preço técnica.
...
export function ProductCard({
  ...
  return (
    <article
      ...
      onClick={() => onView?.(p)}
    >
      <div className="relative aspect-square bg-[var(--sf-surface-muted)] p-2.5">
        <div className="relative h-full w-full overflow-hidden rounded-[var(--sf-radius-sm)] bg-white">
          <img
            src={productThumb(p)}
            alt={p.name}
            loading="lazy"
            className={`h-full w-full object-cover transition-transform duration-300 ${outOfStock ? "opacity-40" : "group-hover:scale-[1.03]"}`}
          />
        </div>
        {isPromo && !outOfStock && (
          <span className="absolute left-2.5 top-2.5 rounded-[var(--sf-radius-pill)] bg-[var(--sf-danger)] px-2 py-1 text-[10px] font-bold text-white">
            -{discountPct}%
          </span>
        )}
        {outOfStock && (
          <span className="absolute inset-0 flex items-center justify-center">
            <span className="rounded-[var(--sf-radius-pill)] bg-[var(--sf-ink)]/80 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-white">
              Esgotado
            </span>
          </span>
        )}
      ...
      <div className="flex flex-1 flex-col gap-1 px-2.5 pb-2.5 pt-1.5">
        ...
        <div className={compact ? "" : "mt-auto"}>
          <div className="flex items-baseline gap-2">
            <span className={`text-[15px] font-semibold leading-none tabular-nums ${isPromo ? "text-[var(--sf-danger)]" : "text-[var(--sf-ink)]"}`}>
          ...
          <div className="mt-1 flex items-center gap-1.5 text-[11px] text-[var(--sf-ink-secondary)]">
            {hasCondition && (
              <span className="rounded-[var(--sf-radius-pill)] bg-[var(--sf-surface-muted)] px-1.5 py-0.5 text-[10px] font-medium text-[var(--sf-ink-secondary)]">
        ...
        {!compact && (
          <button
            ...
            onClick={handleAdd}
            className={`mt-2 flex h-9 w-full items-center justify-center gap-1.5 rounded-[var(--sf-radius-pill)] text-[11px] font-semibold transition-colors ${
              outOfStock
