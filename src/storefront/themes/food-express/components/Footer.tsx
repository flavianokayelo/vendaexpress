// Footer do tema Food Express — horários, endereço e canais de pedido.
// Fundo laranja (--sf-primary), títulos com ponto verde (--sf-accent) e CTA
// de pedido em destaque para conversão rápida.
import { Clock, MapPin, MessageCircle, ShoppingCart } from "lucide-react";
      ...
      <div className="mx-auto grid max-w-[1280px] grid-cols-1 gap-8 px-4 py-10 sm:grid-cols-2 sm:px-6 lg:grid-cols-4">
        ...
        <div className="min-w-0">
          ...
          {waDigits ? (
            <a
              ...
              className="mt-4 inline-flex items-center gap-2 rounded-full bg-[var(--sf-accent)] px-4 py-2.5 text-[13px] font-semibold text-white transition-colors hover:brightness-95"
            >
              <ShoppingCart size={15} /> Pedir agora
            </a>
          ) : null}
        </div>
        ...
        <div>
          <h4 className={titleCls}>
            <span className="h-1.5 w-1.5 rounded-full bg-[var(--sf-accent)]" aria-hidden /> Cardápio
          </h4>
        ...
        <div>
          <h4 className={titleCls}>
            <span className="h-1.5 w-1.5 rounded-full bg-[var(--sf-accent)]" aria-hidden /> Horários &amp; Local
          </h4>
          <ul className="space-y-2 text-[13px] text-white/60">
            <li className="inline-flex items-center gap-2">
              <Clock size={14} className="text-[var(--sf-accent)]" /> Abertos todos os dias
            </li>
            <li className="inline-flex items-center gap-2">
              <MapPin size={14} className="text-[var(--sf-accent)]" /> Pedidos com entrega ao domicílio
            </li>
          </ul>
        </div>
        ...
        <div>
          <h4 className={titleCls}>
            <span className="h-1.5 w-1.5 rounded-full bg-[var(--sf-accent)]" aria-hidden /> Canais de pedido
          </h4>
          <ul className="space-y-2.5 text-[13px] text-white/60">
            {waDigits ? (
              <li>
                ...
                >
                  <MessageCircle size={14} className="text-[var(--sf-accent)]" /> WhatsApp
                </a>
              </li>
            ) : null}
            {waDigits ? <li className="text-white/40">WhatsApp: {store.whatsapp}</li> : <li className="text-white/40">Sem contacto configurado</li>}
          </ul>
        </div>
      </div>
