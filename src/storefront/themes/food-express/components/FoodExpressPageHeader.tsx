// =============================================================================
// FoodExpressPageHeader — header compacto das páginas abertas do tema
// Food Express (restauração). Barra flutuante laranja bem redonda
// (--sf-radius-pill) com ponto verde (--sf-accent) como marcador de entrega.
...
export function FoodExpressPageHeader({ api }: { api: StorefrontApi }) {
      ...
      <div className="mx-auto max-w-6xl px-3 py-2 sm:px-4">
        <div className="flex items-center justify-between gap-3 rounded-[var(--sf-radius-pill)] bg-[var(--sf-primary)] px-3 py-2.5 shadow-[var(--sf-shadow-md)] sm:px-4">
          ...
          <div className="flex min-w-0 flex-shrink-0 items-center gap-2">
            ...
            {store.logo_url ? (
              <span className="flex h-8 w-8 flex-shrink-0 items-center justify-center overflow-hidden rounded-full bg-white p-1">
                <img src={resolveMediaUrl(store.logo_url) ?? ""} alt={store.name} className="h-full w-full object-contain" />
              </span>
            ) : (
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-white font-bold text-[var(--sf-primary)]">
                {store.name[0]}
              </div>
            )}
