// ProductCard do tema Fashion — editorial em retrato (4:5). Nome em maiúsculas,
// "Adicionar" surge no hover, badge de promoção com o --sf-primary (cor da loja).
...
export function ProductCard({
  ...
  return (
    <article
      className={`group relative flex h-full w-full flex-col overflow-hidden bg-[var(--sf-surface)] ${onView ? "cursor-pointer" : ""}`}
      onClick={() => onView?.(p)}
    >
      <div className="relative aspect-[4/5] overflow-hidden bg-[var(--sf-surface-muted)]">
        <img
          src={productThumb(p)}
          alt={p.name}
          loading="lazy"
          className={`h-full w-full object-cover transition-transform duration-300 group-hover:scale-[1.04] ${outOfStock ? "opacity-40" : ""}`}
        />
        {hasCondition && (
          <span className="absolute left-0 top-3 bg-[var(--sf-ink)] px-2 py-1 text-[9px] font-semibold uppercase tracking-[0.12em] text-white">
            {CONDITION_LABEL[p.item_condition]}
          </span>
        )}
        {isPromo && (
          <span className="absolute right-3 top-3 bg-[var(--sf-primary)] px-2 py-1 text-[10px] font-bold uppercase tracking-[0.1em] text-white">
            -{discountPct}%
          </span>
        )}
        {outOfStock && (
          <span className="absolute inset-0 flex items-center justify-center bg-black/10">
            <span className="bg-black/60 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-white">
              Esgotado
            </span>
          </span>
        )}
        {onToggleWishlist && (
          <button
            ...
            className={`absolute bottom-3 left-3 flex h-8 w-8 items-center justify-center bg-white text-[var(--sf-ink-secondary)] shadow-sm transition-colors ${
        ...
        {!outOfStock && (
          <button
            ...
            className={`absolute bottom-3 right-3 flex items-center gap-1.5 px-3 py-2 text-[10px] font-bold uppercase tracking-[0.14em] text-white opacity-0 shadow-md transition-all group-hover:opacity-100 ${
      ...
      <div className="flex flex-1 flex-col gap-1 px-1 pb-1 pt-2.5">
        {!compact && (
          <h3 className="line-clamp-2 text-[11px] font-semibold uppercase leading-[1.5] tracking-[0.1em] text-[var(--sf-ink)]">
            {p.name}
          </h3>
        )}
        ...
        {!compact && categoryName && (
          <span className="truncate text-[10px] uppercase tracking-[0.08em] text-[var(--sf-ink-secondary)]">
            {categoryName}
          </span>
        )}
      </div>
