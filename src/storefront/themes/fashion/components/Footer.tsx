// Footer do tema Fashion — editorial, escuro, tipografia em maiúsculas.
      ...
      <div className="mx-auto max-w-[1280px] px-4 py-12 sm:px-6">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          <div>
            ...
            {store.description && (
              <p className="max-w-[34ch] text-[13px] leading-[1.7] text-white/50">{store.description}</p>
            )}
          </div>

          {categories.length > 0 && (
            <div>
              <div className="mb-4 font-display text-[11px] font-bold uppercase tracking-[0.18em] text-[var(--sf-primary)]">
                Categorias
              </div>
                ...
                {categories.slice(0, 8).map((c) => (
                  <li key={c.id}>
                    <button
                      ...
                      className="text-left text-white/55 uppercase tracking-[0.06em] transition-colors hover:text-white"
                    >
                      {c.name}
                    </button>
                  </li>
                  ...
                  return (
                    <li key={item.title} className="border-b border-white/10 py-2.5 first:pt-0 last:border-b-0">
                      <button
                        ...
                        className="flex w-full items-center justify-between gap-2 text-left text-white/70 uppercase tracking-[0.06em] transition-colors hover:text-white"
                      >
                        {item.title}
                        <ChevronDown
                          size={14}
                      ...
                      {open && <p className="mt-2 text-[12px] leading-[1.7] text-white/45">{item.content}</p>}
                    </li>
                  );
                })}
              </ul>
        ...
        <div className="mt-10 flex flex-col items-center gap-2 border-t border-white/10 pt-6 text-center text-[12px] text-white/40 sm:flex-row sm:justify-between sm:text-left">
