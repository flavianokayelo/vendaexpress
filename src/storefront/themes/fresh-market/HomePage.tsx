// HomePage do tema Fresh Market — mercearia/supermercado. Verde fresco com
// acentos laranja. Announcement → Hero redondo → Categorias circulares →
// Ofertas (rail) → Produtos rápidos (rail) → Catálogo. Compra rápida e de
// baixo atrito, com busca grande no header.
    ...
    <div className="bg-[var(--sf-surface)]">
      ...
      <main className="mx-auto max-w-[1280px] px-4 pb-14 sm:px-6">
        ...
        <div className="mt-4 flex flex-wrap items-center justify-center gap-x-6 gap-y-1 rounded-full bg-[var(--sf-surface-muted)] px-5 py-2.5">
          <span className="text-[11px] font-semibold text-[var(--sf-primary)]">Fresco todos os dias</span>
          <span className="hidden h-3 w-px bg-[var(--sf-line)] sm:block" aria-hidden />
          <span className="text-[11px] font-semibold text-[var(--sf-ink-secondary)]">Envio rápido</span>
          <span className="hidden h-3 w-px bg-[var(--sf-line)] sm:block" aria-hidden />
          <span className="hidden text-[11px] font-semibold text-[var(--sf-ink-secondary)] sm:inline">Abertos todos os dias</span>
        </div>

        {/* hero promocional redondo */}
        {banner && (
          <section
        ...
        {/* categorias em destaque — circulares, cedo e com destaque */}
        ...
        {/* produtos rápidos / destaques em rail */}
        {!selectedCat && !search.trim() && featured.length > 0 && (
          <section className="pb-10">
            <SectionHead title="Produtos rápidos" action={{ label: "Ver todos", onClick: scrollToCatalog }} />
            <ProductGrid
              products={featured}
              currency={currency}
              categoryNames={categoryNames}
        ...
        {/* catálogo completo */}
        <section ref={catalogRef} className="scroll-mt-44 py-8">
          <SectionHead
