// ProductCard do tema Fashion Luxe — editorial 4:5 com foco em fotografia.
...
export function ProductCard({
  ...
  return (
    <article
      className={`group relative flex h-full w-full flex-col overflow-hidden border border-[var(--sf-line)] bg-[var(--sf-surface)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[var(--sf-shadow-sm)] ${onView ? "cursor-pointer" : ""}`}
      onClick={() => onView?.(p)}
    >
      <div className="relative aspect-[4/5] overflow-hidden bg-[var(--sf-surface-muted)]">
        ...
        {isPromo && (
          <span className="absolute right-3 top-3 border border-[var(--sf-accent)]/40 bg-white/95 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.1em] text-[var(--sf-primary)]">
            −{discountPct}%
          </span>
        )}
        {outOfStock && (
      ...
      <div className="flex flex-1 flex-col gap-1 px-3 pb-3 pt-3.5">
        {!compact && (
