// HomePage do tema Fashion Luxe — boutique editorial de luxo acessível.
// Hero editorial grande → Coleção em destaque → Categorias curadas →
// Produtos selecionados → Catálogo completo → Footer institucional.
// Poucas cores, bastante respiro e promoções como curadoria. Consome API.
  ...
  if (orderSuccess) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[var(--sf-surface-muted)] px-6 text-center">
        ...
        <h1 className="font-display text-2xl font-semibold uppercase tracking-wide text-[var(--sf-ink)]">Pedido enviado!</h1>
      ...
      <main className="mx-auto max-w-[1280px] px-4 pb-16 sm:px-6">
        ...
        {banner && (
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-[var(--sf-primary)]/80 via-transparent to-transparent" />
        ...
        {!selectedCat && !search.trim() && featured.length > 0 && (
          <section className="py-14">
            <SectionHead kicker="Nova chegada" title="Coleção em destaque" action={{ label: "Ver todas", onClick: scrollToCatalog }} />
            <ProductGrid
              products={featured}
              currency={currency}
        ...
        {/* produtos selecionados — curadoria */}
        {promoProducts.length > 0 && (
          <section className="pb-14">
            <SectionHead kicker="Curadoria" title="Produtos selecionados" />
            <ProductGrid
