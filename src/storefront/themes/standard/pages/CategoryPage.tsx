// Página aberta de categoria do tema standard. Consome a StorefrontApi (com
...
export function CategoryPage(api: StorefrontApi) {
  const { slug, navigate, store, products, categories, currency, addToCart, isWishlisted, toggleWishlist } =
    api;
  ...
  const availableConditions = useMemo(
    () => Array.from(new Set(categoryProducts.map((p) => p.item_condition))),
    [categoryProducts],
  );
      ...
      <div className="mx-auto max-w-6xl px-4 py-6">
        <div className="mb-4 flex items-center gap-1.5 text-[13px] text-[var(--sf-ink-secondary)]">
          ...
          {category ? (
            ...
            <span className="text-[var(--sf-ink)]">Categoria</span>
          )}
        </div>
