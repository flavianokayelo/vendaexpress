// Página aberta de produto do tema standard. Consome a StorefrontApi (com
...
export function ProductPage(api: StorefrontApi) {
  ...
  const category =
    product?.category_id ? categories.find((c) => c.id === product.category_id) ?? null : null;
  const related = product
    ? products.filter((p) => p.id !== product.id && p.category_id === product.category_id).slice(0, 8)
    : [];
  ...
  const accent = store.theme_primary || "#4F46E5";
  const isPromo =
  ...
  const videoThumb = product.video?.thumbnail_url
    ? resolveMediaUrl(product.video.thumbnail_url)
    : null;
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 pb-24 sm:pb-6">
        <div className="grid gap-8 md:grid-cols-2">
            ...
            >
              {activeMedia.type === "video" ? (
                <>
                  ...
                  <span className="absolute bottom-3 left-3 rounded-[var(--sf-radius-pill)] bg-black/50 px-3 py-1 font-display text-[12px] font-semibold uppercase tracking-[0.06em] text-white">
                    Vídeo
                  </span>
                </>
              ) : (
          ...
          <div>
            ...
            {product.description && (
              <p className="mt-4 whitespace-pre-line text-sm leading-relaxed text-[var(--sf-ink-secondary)]">
                {product.description}
              </p>
            )}
