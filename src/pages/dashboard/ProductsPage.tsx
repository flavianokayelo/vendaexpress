...
import {
  Plus, Package, Pencil, Trash2, Search, Sparkles, X, ImagePlus, Check, Video, Film,
} from 'lucide-react';
import { api, uploadProductImages, uploadProductVideo, aiAssistImage, resolveMediaUrl } from '../../lib/api';
      ...
      <div className="flex flex-wrap gap-2">
        {SIZE_PRESETS.map((s) => (
          <button
            key={s}
            type="button"
            onClick={() => { setCustomMode(false); onChange(s); }}
        ...
        <button
          ...
          className={`rounded-lg border px-3 py-1.5 text-sm font-medium transition-colors ${
            customMode ? 'border-blue-700 bg-blue-50 text-blue-700' : 'border-slate-200 text-slate-600 hover:border-slate-300'
          }`}
        >
          Outro
        </button>
...
function ProductCard({
  ...
  const thumb = resolveMediaUrl(p.images?.[0]?.url ?? p.image_url) || placeholderImage(p.name);
    ...
    <div className="group overflow-hidden rounded-2xl border border-slate-200 bg-white">
      ...
      >
        ...
        <div className="absolute left-2 top-2 flex flex-col gap-1">
          {p.compare_at_price && p.compare_at_price > p.price && <Badge color="red">Promoção</Badge>}
          {videoUrl && (
            <span
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="text-right">
            <div className="font-bold text-blue-800">{formatCurrency(Number(p.price), currency)}</div>
            {p.compare_at_price && p.compare_at_price > p.price && (
              <div className="text-xs text-slate-400 line-through">{formatCurrency(Number(p.compare_at_price), currency)}</div>
            )}
          </div>
        </div>
        <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-slate-500">
          ...
          <Badge color={p.item_condition === 'novo' ? 'green' : 'amber'}>{CONDITION_LABEL[p.item_condition]}</Badge>
        </div>
        <div className="mt-3 flex gap-2">
          <Button size="sm" variant="outline" onClick={() => onEdit(p)}><Pencil size={14} /> Editar</Button>
          <Button size="sm" variant="ghost" onClick={() => onRemove(p)} className="text-red-600 hover:bg-red-50"><Trash2 size={14} /></Button>
        </div>
      </div>
    </div>
  );
...
export function ProductsPage() {
  ...
  const [aiImageFromCache, setAiImageFromCache] = useState<boolean | null>(null);
  ...
  const openEdit = (p: Product) => {
    ...
    setImages((p.images ?? []).map((photo) => ({ url: photo.url, hash: photo.hash, isNew: false })));
    setVideo(p.video ? { url: p.video.url, thumbnailUrl: p.video.thumbnail_url, isNew: false } : null);
    setVideoError(null);
    resetAiState();
    setError(null);
    setModalOpen(true);
  ...
  const handleVideoSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (sizeMB > MAX_VIDEO_MB) {
      setVideoError(`O vídeo tem ${sizeMB.toFixed(1)}MB — o limite é ${MAX_VIDEO_MB}MB.`);
      return;
    }
  ...
  const runAiTextAssist = async () => {
    ...
    try {
      ...
      const { name, description } = await api.products.aiAssist(aiKeywords, categoryName);
  ...
  const runAiImageAssist = async () => {
    ...
    setAiImageFromCache(null);
    try {
      const { name, description, color, category, from_cache } = await aiAssistImage(firstNew.file);
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const finalImages = images.map((img) =>
        img.isNew ? uploadedPhotos[uploadIdx++] : { url: img.url, hash: img.hash }
      );

      let finalVideo: { url: string; thumbnail_url: string | null } | null = null;
      if (video?.isNew && video.file) {
        const res = await uploadProductVideo(video.file, video.thumbnailBlob ?? null);
        finalVideo = res;
  ...
  return (
    <div>
      <PageHeader
        ...
        action={<Button onClick={openNew}><Plus size={16} /> Adicionar produto</Button>}
      />
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((p) => (
            <ProductCard key={p.id} p={p} currency={store?.currency} onEdit={openEdit} onRemove={remove} />
          ))}
        </div>
      )}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar produto' : 'Novo produto'} size="lg">
        <form onSubmit={save} className="space-y-4">
          ...
          <Field label="Fotos do produto" hint={`Até ${MAX_PHOTOS} imagens. ${images.length}/${MAX_PHOTOS} selecionadas.`}>
            <div className="flex flex-wrap gap-3">
              {images.map((img, idx) => (
                <div key={idx} className="relative h-20 w-20">
                  <img src={img.isNew ? img.url : (resolveMediaUrl(img.url) ?? '')} alt="" className="h-20 w-20 rounded-lg border border-slate-200 object-cover" />
                  <button
                    type="button"
            ...
            {uploading && <p className="mt-1 text-xs text-blue-600">A enviar imagens...</p>}
          </Field>

          <Field label="Vídeo do produto" hint={`Opcional. Até ${MAX_VIDEO_MB}MB (mp4, webm ou mov).`}>
            {video ? (
              <div className="flex items-start gap-3">
                <div className="relative aspect-video w-48 overflow-hidden rounded-lg border border-slate-200 bg-slate-100">
                  <video
                    src={video.isNew ? video.url : (resolveMediaUrl(video.url) ?? '')}
                    poster={video.isNew ? (video.thumbnailUrl ?? undefined) : (resolveMediaUrl(video.thumbnailUrl) ?? undefined)}
                    controls
                    preload="metadata"
          ...
          <div className="space-y-3 rounded-xl border border-blue-100 bg-blue-50/60 p-3">
            ...
            <div>
              ...
              {aiImageError && <p className="mt-1 text-xs text-red-600">{aiImageError}</p>}
              {aiImageFromCache !== null && !aiImageError && (
                <p className="mt-1 flex items-center gap-1 text-xs text-green-600">
                  <Check size={12} />
                  {aiImageFromCache ? 'Resultado veio da cache (0 créditos gastos)' : 'Analisado agora com IA'}
                </p>
              )}
            </div>

            <div>
              <div className="mb-1 text-xs font-medium text-slate-600">Ou por palavras-chave</div>
              <div className="flex gap-2">
                <Input
                  ...
                  className="flex-1"
                />
                <Button type="button" variant="outline" onClick={runAiTextAssist} disabled={aiTextBusy}>
                  {aiTextBusy ? 'A gerar...' : 'Gerar'}
                </Button>
              </div>
              {aiTextError && <p className="mt-1 text-xs text-red-600">{aiTextError}</p>}
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Estado">
              <Select value={form.item_condition} onChange={(e) => setForm({ ...form, item_condition: e.target.value as ProductCondition })}>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Cor" hint="Opcional">
              <ColorPicker
                ...
                onChange={(name, hex) => setForm({ ...form, color: name, color_hex: hex })}
              />
            </Field>
            <Field label="Tamanho" hint="Opcional">
              <SizePicker value={form.size} onChange={(size) => setForm({ ...form, size })} />
            </Field>
          </div>
