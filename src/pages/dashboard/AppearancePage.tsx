    ...
    try {
      ...
      setUploadError(
        err instanceof Error
          ? err.message
          : "Erro ao enviar as fotos do banner",
      );
    } finally {
    ...
    <div>
      ...
      <form onSubmit={save} className="max-w-2xl space-y-6">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3">
            {bannerUrls.map((url) => (
              <div
                key={url}
                className="group relative h-24 w-40 overflow-hidden rounded-xl border border-slate-200"
              >
                <img
        ...
        <div className="rounded-2xl border border-slate-200 overflow-hidden">
          {bannerUrls[0] ? (
            <img
              src={resolveMediaUrl(bannerUrls[0]) ?? ""}
              alt="Banner preview"
